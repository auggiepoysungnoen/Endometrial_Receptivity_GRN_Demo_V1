...
(async function () {
  ...
  function evidenceHTML(id, A) {
    ...
    const row = (r) => `<tr>
        <td class="ev-ref">${pubmed(r.pmid)}<span class="ev-cite">${esc(r.author ? r.author + " et al." : "—")}${r.year ? ` (${esc(r.year)})` : ""}</span>${r.journal ? `<span class="ev-jrn">${esc(r.journal)}</span>` : ""}</td>
        <td>${cell(r)}</td></tr>`;
    const first = rows.slice(0, 4), rest = rows.slice(4);
    ...
    return `<table class="evid">
      ...
      <tbody>${first.map(row).join("")}</tbody>
      ${rest.length ? `<tbody class="ev-rest" hidden>${rest.map(row).join("")}</tbody>` : ""}
    </table>
    <p class="ev-foot">${rest.length ? `<button class="linklike ev-more" data-n="${rest.length}">Show ${rest.length} more</button> · ` : ""}<a href="${search}" target="_blank" rel="noopener">All ${A.nImpl} on PubMed</a></p>`;
  }
  ...
  function confirmedHTML(id) {
    ...
    return `<table class="evid">
      ...
      <tbody>${known.slice(0, 6).map((k) => `<tr>
        <td class="ev-ref"><b>${esc(k.s)}</b> → ${esc(k.t)}</td>
        <td>${k.lit.sign === "+" ? "switches on" : k.lit.sign === "-" ? "switches off" : "regulates"}
          <span class="pmids">${k.lit.refs.slice(0, 2).map(pubmed).join("")}${k.lit.nRefs > 2 ? `<span class="more-refs">+${k.lit.nRefs - 2}</span>` : ""}</span></td></tr>`).join("")}</tbody>
  ...
  function aboutHTML(id, n) {
    ...
    if (A.note) {
      ...
      body2 = `<p class="gtext">${esc(A.note.text)}</p>
        <p class="note-src"><span class="status ${reviewed ? "reviewed" : ""}" title="${reviewed ? "Checked by the lab" : "Written from the cited papers; not yet checked by the lab"}">${reviewed ? `Reviewed${A.note.reviewer ? " · " + esc(A.note.reviewer) : ""}` : "Draft"}</span> · sources in E</p>`;
  ...
  detail.addEventListener("click", (ev) => {
    const evMore = ev.target.closest(".ev-more");
    if (evMore) {
      const rest = evMore.closest(".gsec-body").querySelector(".ev-rest");
      const open = rest.hidden;
      rest.hidden = !open;
      evMore.textContent = open ? "Show fewer" : `Show ${evMore.dataset.n} more`;
      return;
    }
